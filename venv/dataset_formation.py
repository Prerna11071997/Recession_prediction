...
files = ['GoldPMPrice.csv', 'unemp_rate.csv', 'DollarExchangeRate.csv', 'NASDAQ_indice.csv', '3YearTBill.csv', '1YearTBill.csv',
         'GDP_growth.csv', 'UrbanConsCPI.csv']
...
print(recession_indicator)
recession_indicator.to_csv("base.csv")
...
for i in os.listdir("./data"):
    if i != '3YearTBill.csv' and i != 'NASDAQ_indice.csv' and i != 'leading_indicators.csv' and i != 'recession_state.csv':
        # Useless data are removed here+
        p.append(pd.read_csv("./data/" + i))

#p.append(recession_indicator)
...
meta_days = []
for i in p:
    meta_date.append([(i[0:1].index[0].date()), i.columns[0]])
# Starting dates of all columns
for i in p:
    meta_days.append([(i[1:2].index[0] - i[0:1].index[0]).days, i.columns[0]])  # contains pairs of days and name of the column
